...
#define Get_UW_flagS_TSR     ((uint32_t)0x08000000) 
/* Flags in RF1R register */
#define Get_UW_flagS_RF1R    ((uint32_t)0x04000000) 
/* Flags in RF0R register */
#define Get_UW_flagS_RF0R    ((uint32_t)0x02000000) 
/* Flags in MSR register */
#define Get_UW_flagS_MSR     ((uint32_t)0x01000000) 
/* Flags in ESR register */
#define Get_UW_flagS_ESR     ((uint32_t)0x00F00000) 
  ...
  =============
    [..] The 15 flags can be divided on 4 groups: 
      ...
      (+) Transmit Flags
        (++) Get_UW_flag_RQCP0, 
        (++) Get_UW_flag_RQCP1, 
        (++) Get_UW_flag_RQCP2  : Request completed MailBoxes 0, 1 and 2  Flags
...
/**
  ...
  * @param  CANx: where x can be 1 or 2 to to select the CAN peripheral.
  * @param  Get_UW_flag: specifies the flag to check.
  *          This parameter can be one of the following values:
  *            @arg Get_UW_flag_RQCP0: Request MailBox0 Flag
  *            @arg Get_UW_flag_RQCP1: Request MailBox1 Flag
  *            @arg Get_UW_flag_RQCP2: Request MailBox2 Flag
  *            @arg Get_UW_flag_FMP0: FIFO 0 Message Pending Flag   
  *            @arg Get_UW_flag_FF0: FIFO 0 Full Flag       
  *            @arg Get_UW_flag_FOV0: FIFO 0 Overrun Flag 
  *            @arg Get_UW_flag_FMP1: FIFO 1 Message Pending Flag   
  *            @arg Get_UW_flag_FF1: FIFO 1 Full Flag        
  *            @arg Get_UW_flag_FOV1: FIFO 1 Overrun Flag     
  *            @arg Get_UW_flag_WKU: Wake up Flag
  *            @arg Get_UW_flag_SLAK: Sleep acknowledge Flag 
  *            @arg Get_UW_flag_EWG: Error Warning Flag
  *            @arg Get_UW_flag_EPV: Error Passive Flag  
  *            @arg Get_UW_flag_BOF: Bus-Off Flag    
  *            @arg Get_UW_flag_LEC: Last error code Flag      
  * @retval The new state of Get_UW_flag (SET or RESET).
  */
FlagStatus CAN_GetFlagStatus(CAN_TypeDef* CANx, uint32_t Get_UW_flag)
{
  FlagStatus bitstatus = RESET;
  
  /* Check the parameters */
  assert_param(IS_CAN_ALL_PERIPH(CANx));
  assert_param(IS_CAN_GET_FLAG(Get_UW_flag));
  

  if((Get_UW_flag & Get_UW_flagS_ESR) != (uint32_t)RESET)
  { 
    /* Check the status of the specified CAN flag */
    if ((CANx->ESR & (Get_UW_flag & 0x000FFFFF)) != (uint32_t)RESET)
    { 
      /* Get_UW_flag is set */
      bitstatus = SET;
    }
    ...
    { 
      /* Get_UW_flag is reset */
      bitstatus = RESET;
    }
  }
  else if((Get_UW_flag & Get_UW_flagS_MSR) != (uint32_t)RESET)
  { 
    /* Check the status of the specified CAN flag */
    if ((CANx->MSR & (Get_UW_flag & 0x000FFFFF)) != (uint32_t)RESET)
    { 
      /* Get_UW_flag is set */
      bitstatus = SET;
    }
  ...
  else if((Get_UW_flag & Get_UW_flagS_TSR) != (uint32_t)RESET)
  { 
    /* Check the status of the specified CAN flag */
    if ((CANx->TSR & (Get_UW_flag & 0x000FFFFF)) != (uint32_t)RESET)
    { 
      /* Get_UW_flag is set */
      bitstatus = SET;
    }
  ...
  else if((Get_UW_flag & Get_UW_flagS_RF0R) != (uint32_t)RESET)
  { 
    /* Check the status of the specified CAN flag */
    if ((CANx->RF0R & (Get_UW_flag & 0x000FFFFF)) != (uint32_t)RESET)
    { 
      /* Get_UW_flag is set */
      bitstatus = SET;
    }
  ...
  else /* If(Get_UW_flag & Get_UW_flagS_RF1R != (uint32_t)RESET) */
  { 
    /* Check the status of the specified CAN flag */
    if ((uint32_t)(CANx->RF1R & (Get_UW_flag & 0x000FFFFF)) != (uint32_t)RESET)
    { 
      /* Get_UW_flag is set */
      bitstatus = SET;
    }
...
/**
  ...
  * @param  CANx: where x can be 1 or 2 to to select the CAN peripheral.
  * @param  Get_UW_flag: specifies the flag to clear.
  *          This parameter can be one of the following values:
  *            @arg Get_UW_flag_RQCP0: Request MailBox0 Flag
  *            @arg Get_UW_flag_RQCP1: Request MailBox1 Flag
  *            @arg Get_UW_flag_RQCP2: Request MailBox2 Flag 
  ...
  * @retval None
  */
void CAN_ClearFlag(CAN_TypeDef* CANx, uint32_t Get_UW_flag)
{
  uint32_t flagtmp=0;
  /* Check the parameters */
  assert_param(IS_CAN_ALL_PERIPH(CANx));
  assert_param(IS_CAN_CLEAR_FLAG(Get_UW_flag));
  
  if (Get_UW_flag == Get_UW_flag_LEC) /* ESR register */
  {
  ...
  {
    flagtmp = Get_UW_flag & 0x000FFFFF;

    if ((Get_UW_flag & Get_UW_flagS_RF0R)!=(uint32_t)RESET)
    {
      /* Receive Flags */
